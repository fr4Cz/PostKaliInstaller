...
/usr/sbin/useradd -u ${PKIUID} -g staff -G sudo,bluetooth,cdrom,audio -d /home/${PKIUSR} -s /bin/bash -p $(/usr/bin/echo ${PKIPSW} | /usr/bin/openssl passwd -1 -stdin) ${PKIUSR} 2>> ${PKILOG}
