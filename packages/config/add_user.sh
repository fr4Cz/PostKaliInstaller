...
/usr/sbin/useradd -u ${PKIUID} -g staff,sudo,bluetooth,beef,wireshark,postgres,cdrom,audio,kismet -d /home/${PKIUSR} -s /bin/bash -p $(/usr/bin/echo ${PKIPSW} | /usr/bin/openssl passwd -1 -stdin) ${PKIUSR} 2>> ${PKILOG}
